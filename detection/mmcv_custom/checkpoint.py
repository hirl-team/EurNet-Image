...
def load_state_dict(module, state_dict, strict=False, logger=None):
    ...
    if len(err_msg) > 0 and rank == 0:
        ...
        if strict:
            ...
            print(err_msg)
    elif len(err_msg) == 0 and rank == 0:
        print("==> all keys are matched sucessfully!!!")
...
def load_checkpoint(model,
    ...
    if state_dict.get('absolute_pos_embed') is not None:
        ...
        if N1 != N2 or C1 != C2 or L != H*W:
            logger.warning("Interpolate absolute_pos_embed to model absolute pos embed shape")
            absolute_pos_embed = absolute_pos_embed.resize(N1, int(np.sqrt(L)), int(np.sqrt(L)), C1).permute(0,3,1,2).contiguous()
            state_dict['absolute_pos_embed'] = F.interpolate(absolute_pos_embed, size=(H, W), mode='bicubic')
        else:
            state_dict['absolute_pos_embed'] = absolute_pos_embed.view(N2, H, W, C2).permute(0, 3, 1, 2)

    rank, _ = get_dist_info()
    all_keys = list(state_dict.keys())
    for key in all_keys: # remove relative pos in state dict
        if "relative_pos" in key:
            state_dict.pop(key)

    # context layers: load kernel weights at center and zero-pad surrounding
    context_layers_keys = [k for k in state_dict.keys() if ("context_layers" in k and "bias" not in k)]
    for table_key in context_layers_keys:      
        print("processing context layers {}".format(table_key))
        ...
        if len(table_pretrained.shape) != 4:
            ...
            print("resizing pretrained key {} from shape {} to {}".format(table_key, L1, L2))
            if L1 != L2:
                S1 = int(L1 ** 0.5)
                S2 = int(L2 ** 0.5)
            ...
            print("resizing pretrained key {} from shape {} to {}".format(table_key, fsize1, fsize2))
            if fsize1 < fsize2:
    ...
    # load relation weight linear layer
    weight_layers_keys = [k for k in state_dict.keys() if ("weight_linear" in k)]
    for table_key in weight_layers_keys:        
        if table_key not in model.state_dict():
            continue
        table_pretrained = state_dict[table_key]
        table_current = model.state_dict()[table_key]
        if table_pretrained.shape != table_current.shape:
            if len(table_pretrained.shape) == 2:
                ...
                if L1 < L2:
                    table_pretrained_resized = torch.zeros(table_current.shape)
                    # copy for graph relation
                    table_pretrained_resized[:(L1-1)] = table_pretrained[:(L1-1)]
                    # copy for context relation
