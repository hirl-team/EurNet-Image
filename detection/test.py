...
from eurnet_det import *

def parse_args():
    parser = argparse.ArgumentParser(
        description='MMDet test (and eval) a model')
    parser.add_argument('config', help='test config file path')
    parser.add_argument('checkpoint', type=str, nargs="+", help='checkpoint file')
...
def main():
    ...
    for ckpt in args.checkpoint:
        ...
        if not distributed:
            ...
            outputs = multi_gpu_test(ddp_model, data_loader, args.tmpdir,
                                    args.gpu_collect)
    ...
    rank, _ = get_dist_info()
    if rank == 0:
        print("best checkpoint {}, results: {}".format(best_ckpt, best_result))
