...
class EurNetBlock(nn.Module):
    ...
    Args:
        ...
        context_relation (bool): Whether to add an extra focal relation.
        context_sizes (list of int): Sizes of focal convolutional kernels.
    ...
    def forward(self, graph, x):
        ...
        shortcut = x
        x = self.norm1(x)
        x = self.graph_conv(x, graph.edge_index, graph.edge_type, graph.edge_weight, size, B, graph.num_relation)
        x = self.proj(x)
...
class EurNetStage(nn.Module):
    ...
    def forward(self, x, checkpoint_graph=None):
        ...
        if checkpoint_graph is not None:
            ...
            data_list = [Data(x=x[data_id]) for data_id in range(B)]
